...
function Profile() {
    ...
    const handleSubmitSpecialist = async (event) => {
      ...
      if(values.name != '') {
        ...
        await db.collection('specialists').doc(user.uid).update({name: values.name});
      } if (values.email != '') {
        user.email = values.email; // Se actualiza en el context
        // Se actualiza en el módulo de autenticación
        await auth.currentUser.updateEmail(values.email);
        // Se actualiza en la base de datos
        await db.collection('users').doc(user.uid).update({email: values.email});
        await db.collection('specialists').doc(user.uid).update({email: values.email});
      } if (values.phone != '') {
        user.phone = values.phone;
        await db.collection('users').doc(user.uid).update({phone: values.phone});
        await db.collection('specialists').doc(user.uid).update({phone: values.phone});
      } if (values.password != '') {
        await auth.currentUser.updatePassword(values.password);
      } if (values.description != '') {
        user.description = values.description;
        await db.collection('users').doc(user.uid).update({description: values.description});
        await db.collection('specialists').doc(user.uid).update({description: values.description});
      }
      ...
      if(values.speciality != '') {
        ...
        // Se actualiza en la base de datos
        await db.collection('users').doc(user.uid).update({speciality: specialityArray});
        await db.collection('specialists').doc(user.uid).update({speciality: specialityArray});
      }
      if(values.payment != '') {
        user.payment = values.payment; // Se actualiza en el userContext
        // Se actualiza en la base de datos
        await db.collection('users').doc(user.uid).update({payment: values.payment});
        await db.collection('specialists').doc(user.uid).update({payment: values.payment});
      }
      if((values.time_start != '') && (values.time_end != '')) {
        ...
        await db.collection('specialists').doc(user.uid).update({hours: hoursArray});
      }
      ...
      await db.collection('specialists').doc(user.uid).update({work: daysOfWork});
      editProfile(false);
      checkStatus();
    }
    ...
    const checkStatus = async () => {
      ...
      if(counter == 5) {
        ...
        await db.collection('specialists').doc(user.uid).update({status: 'yes'});
    ...
    const handleSubmitPacient = async (event) => {
      ...
      if(values.name != '') {
        ...
        // Se actualiza en la base de datos
        await db.collection('users').doc(user.uid).update({name: values.name});
        await db.collection('pacients').doc(user.uid).update({name: values.name});
      } if (values.email != '') {
        user.email = values.email; // Se actualiza en el context
        // Se actualiza en el módulo de autenticación
        await auth.currentUser.updateEmail(values.email);
        // Se actualiza en la base de datos
        await db.collection('users').doc(user.uid).update({email: values.email});
        await db.collection('pacients').doc(user.uid).update({email: values.email});
      } if (values.phone != '') {
        user.phone = values.phone;
        await db.collection('users').doc(user.uid).update({phone: values.phone});
        await db.collection('pacients').doc(user.uid).update({phone: values.phone});
      } if (values.password != '') {
        await auth.currentUser.updatePassword(values.password);
      } if (values.description != '') {
        user.description = values.description;
        await db.collection('users').doc(user.uid).update({description: values.description});
        await db.collection('pacients').doc(user.uid).update({description: values.description});
      }
      editProfile(false);
    };
        ...
        <div className={styles.profile}>
          ...
          {(user.status === "no") && (
            <div className={styles.uncompletedInfo}>
              ...
              <p>
                Rellene los datos faltantes. De no hacerlo, los 
                pacientes no podrán localizarlo.
              </p>
            </div>
          )}
          <div className={styles.profileInformation}>
              ...
              <form action="submit" onSubmit={handleSubmitSpecialist}>
                <div className={styles.formProfile}>
                  ...
                  <div className={styles.firstColumn}>
                    ...
                    <div className={styles.inputProfile}>
                      <label className={styles.profileTitle} htmlFor="speciality">Especialidades (,):</label>
                      <input
                        name="speciality"
                        id="speciality"
                        type="text"
                    ...
                    <div className={styles.inputProfile}>
                      <label className={styles.profileTitle} htmlFor="description">Costo de la consulta:</label>
                      <input
                        name="payment"
                        id="payment"
                        type="text"
                  ...
                  <div className={styles.secondColumn}>
                    <div className={styles.inputProfile}>
                      <label className={styles.profileTitle} htmlFor="email">Horario de trabajo:</label>
                      <input
                        name="time_start"
                        id="time_start"
                        type="time"
                    ...
                    <div className={styles.inputProfile}>
                      <p className={styles.profileTitle} htmlFor="email">Días laborales:</p>
                      <div className={styles.checkboxWork}>
                        <input name="lunes" id="lunes" type="checkbox" value="1"/>
                        <label htmlFor="lunes">Lunes</label>
                        <input name="martes" id="martes" type="checkbox" onChange={handleOnChange} value="2"/>
                        <label htmlFor="martes">Martes</label>
                        <input name="miercoles" id="miercoles" type="checkbox" onChange={handleOnChange} value="3"/>
                        <label htmlFor="miercoles">Miércoles</label>
                        <input name="jueves" id="jueves" type="checkbox" onChange={handleOnChange} value="4"/>
                        <label htmlFor="jueves">Jueves</label>
                        <input name="viernes" id="viernes" type="checkbox" onChange={handleOnChange} value="5"/>
                        <label htmlFor="viernes">Viernes</label>
                        <input name="sabado" id="sabado" type="checkbox" onChange={handleOnChange} value="6"/>
                        <label htmlFor="sabado">Sábado</label>
                        <input name="domingo" id="domingo" type="checkbox" onChange={handleOnChange} value="0"/>
